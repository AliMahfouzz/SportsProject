...
$query = "SELECT * FROM events WHERE idevents = '$idacademy'";
...
<body>
    ...
    <div class="p-5 row m-0 position-relative justify-content-between">
        <div class="col-md-5 card border-right border-gray">
            <div class="rounded  p-3">
                ...
                <div class="form-group ">
                    ...
                    <span class=" text-muted"><?php echo $duration; ?></span>
                </div>
                <div class="form-group ">
                ...
                <div class="form-group ">
                    ...
                    <span class=" text-muted"><?php echo $age; ?></span>
                </div>
                <div class="form-group ">
        ...
        <div class="col-md-6 card ml-1">
            <?php
            if (isset($_SESSION["eemessage"])) {
                echo '<div class="alert alert-primary m-1">
                                                ' . $_SESSION["eemessage"] . '
                                                </div>';
            } else if (isset($_SESSION["e_eemessage"])) {
                echo '<div class="alert alert-danger m-1">
                                                ' . $_SESSION["e_eemessage"] . '
                                                </div>';
            }

            ?>
            <form method="POST" action="eventdetails.php?id=<?php echo $idacademy ?>" class="rounded  p-3">
                <div>
                ...
                <input type="hidden" name="idevent" value="<?php echo $idacademy; ?>" class="form-control form-control-sm">
                <!-- </div> -->
...
if (isset($_POST['submit'])) {
    ...
    $query = "insert into users_events (idusers, idevents) values ('$idusers','$idevent')";
    $result = mysqli_query($con, $query);

    if ($result) {
        $_SESSION['eemessage'] = 'You are subscribed successfully';
        header('Location: eventdetails.php?id=' . $idevent);
    } else {
        $_SESSION['e_eemessage'] = 'You are not subscribed, try again !!!!';
        header('Location: eventdetails.php?id=' . $idevent);
    }
}
